  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setShowEditForm(false);
        setEditItem(null); // Clear form
      } else {
        console.error("Failed to update FAQ:", response.statusText);
      }
    } catch (error) {
      console.error("Error updating FAQ:", error);
    }
  ...
  const handleNewSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setShowAddForm(false);
        setNewItem({ image: "", title: "", description: "" }); // Clear form
      } else {
        console.error("Failed to create FAQ:", response.statusText);
        setFaqItems((prevFaqItems) =>
          prevFaqItems.filter((item) => item.id !== optimisticFaq.id)
        );
      }
